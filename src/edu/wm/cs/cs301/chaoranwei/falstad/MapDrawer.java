...
public class MapDrawer extends DefaultViewer {
	...
	public void redraw(GraphicsWrapper gc, int state, int px, int py,
		...
		if (state != Constants.STATE_PLAY)
			return ;
		if (maze.isInMapMode()) {
			draw_map(gc, px, py, walk_step, view_dx, view_dy, maze.isInShowMazeMode(), maze.isInShowSolutionMode()) ;
	 ...
	 */
	public void draw_map(GraphicsWrapper gc, int px, int py, int walk_step, int view_dx, int view_dy, boolean showMaze, boolean showSolution) {
	 ...
	 * @param gc
	 */
	public void draw_currentlocation(GraphicsWrapper gc, int view_dx, int view_dy) {
	 ...
	 * @param offy
	 */
	public void draw_solution(GraphicsWrapper gc, int offx, int offy, int px, int py) {
		// check parameters:
