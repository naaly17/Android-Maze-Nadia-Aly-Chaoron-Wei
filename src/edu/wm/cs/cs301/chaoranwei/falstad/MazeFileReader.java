...
public class MazeFileReader {
	 ...
	 *  
	 * @param stream with data of a Maze object
	 */
	public MazeFileReader(String file) {
		load(file) ;
	}
		...
		try{
			...
			for (int temp = 0 ; temp < nList.getLength() ; temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					endx =  getElementIntValue("endx", eElement);
					endy = getElementIntValue("endy", eElement);
					c1 = getElementIntValue("c1", eElement);
					c2 = getElementIntValue("c2", eElement);
					c3 = getElementIntValue("c3", eElement);
				}
			}
		}
